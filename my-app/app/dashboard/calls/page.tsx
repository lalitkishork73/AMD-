            ...
            <div className="bg-white w-full max-w-sm p-6 rounded-2xl shadow-lg border border-gray-200 transition-all">
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="flex items-center justify-between bg-gray-100 rounded-xl px-4 py-3 shadow-inner border border-gray-200 transition-all">
                        <span
                            className={`text-2xl font-mono text-gray-800 truncate ${formData.number ? "animate-pulse text-blue-700" : "text-gray-500"
                                }`}
                        >
                    ...
                    <div className="grid grid-cols-3 gap-3 mt-4">
                        {dialPad.map((digit) => (
                            <button
                                ...
                                className={`text-lg font-semibold rounded-xl py-3 transition-all duration-150 shadow-sm active:scale-95 ${activeKey === digit
                    ...
                    <div className="flex justify-center mt-4">
                        <button
                            type="submit"
                            disabled={loading}
                            className={`flex items-center gap-2 px-8 py-2.5 rounded-full text-white text-base font-medium transition-all shadow-md active:scale-95 ${loading
                    ...
                    {message && (
                        <div
                            className={`mt-2 text-center text-sm font-medium ${message.startsWith("📞")
                                    ? "text-green-600"
                                    : "text-red-600"
                                }`}
                        >
